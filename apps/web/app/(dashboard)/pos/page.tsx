...
export default function POSPage() {
  ...
  const checkout = () => {
    ...
    saleMutation.mutate({
      ...
      paymentMethod,
      amountPaid: paymentMethod === 'MPESA' ? total : Number(amountPaid),
    });
  };
    ...
    <>
      ...
      <div className="flex flex-col lg:flex-row gap-4 h-full">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 overflow-auto pb-4">
            {products.map((p: any) => {
        ...
        <div className="w-full lg:w-80 xl:w-96 flex flex-col gap-3">
          <div className="bg-white rounded-xl border border-gray-200 flex-1 flex flex-col overflow-hidden">
              ...
              {cart.map(item => (
                <div key={item.productId} className="flex items-center gap-2 bg-gray-50 rounded-xl p-2">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => updateQty(item.productId, -1)} className="w-7 h-7 rounded-lg bg-white border flex items-center justify-center hover:bg-gray-100"><Minus size={12} /></button>
                    <span className="w-8 text-center text-sm font-semibold">{item.quantity}</span>
                    <button onClick={() => updateQty(item.productId, 1)} className="w-7 h-7 rounded-lg bg-white border flex items-center justify-center hover:bg-gray-100"><Plus size={12} /></button>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold">{formatKES(item.unitPrice * item.quantity)}</p>
                    <button onClick={() => setCart(c => c.filter(i => i.productId !== item.productId))} className="text-red-400 hover:text-red-600"><Trash2 size={12} /></button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 border-t border-gray-100 space-y-2">
              <div className="flex justify-between text-sm text-gray-500"><span>Subtotal</span><span>{formatKES(subtotal)}</span></div>
              <div className="flex justify-between text-sm text-gray-500"><span>VAT</span><span>{formatKES(vatTotal)}</span></div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
              ...
              {[
                ...
                <button key={method} onClick={() => setPaymentMethod(method as any)}
                  className={`flex flex-col items-center gap-1 py-2 rounded-xl border text-xs font-medium transition-colors ${
                    paymentMethod === method ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                  }`}>
                  <Icon size={16} />{label}
                </button>
              ))}
              ...
              <>
                ...
                {Number(amountPaid) > 0 && (
                  <div className={`flex justify-between text-sm font-semibold px-3 py-2 rounded-xl ${change >= 0 ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                    <span>Change</span><span>{formatKES(Math.max(0, change))}</span>
                  </div>
                )}
              </>
            )}
            ...
            {paymentMethod === 'MPESA' && (
              <div className="bg-green-50 rounded-xl p-3 text-center">
                <p className="text-green-700 text-sm font-medium">💚 M-Pesa STK Push</p>
                <p className="text-green-600 text-xs mt-1">Customer will receive prompt on their phone</p>
              </div>
            )}

            <button onClick={checkout}
              disabled={cart.length === 0 || (paymentMethod !== 'MPESA' && !amountPaid) || saleMutation.isPending}
