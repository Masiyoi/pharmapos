...
import {
  ...
  Package, Calendar, TrendingDown, DollarSign,
} from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const { data: profitData } = useQuery({
    queryKey: ['report-profit', profitDays],
    queryFn: () =>
      api.get(`/reports/profit-analysis?days=${profitDays}`).then(r => r.data),
  });
  ...
  name === 'revenue' ? 'Revenue' : 'VAT',
      ...
      {/* ── NET PROFIT SUMMARY + DETAILED PRODUCT PROFIT ANALYSIS ─────────── */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="font-semibold text-gray-800">Profit & Sales Report</h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Period: last {profitDays} days — actual profit based on cost price at time of sale
            </p>
          </div>
          <div className="flex gap-1">
            {[7, 14, 30].map(d => (
              <button key={d} onClick={() => setProfitDays(d)}
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                  profitDays === d
                    ? 'bg-emerald-600 text-white'
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
          <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs font-medium text-emerald-700">Total Sales Revenue</p>
            </div>
            <p className="text-2xl font-bold text-emerald-800">
              {formatKES(totals?.totalRevenue || 0)}
            </p>
            <p className="text-xs text-emerald-600 mt-0.5">Total money received</p>
          </div>

          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
            <div className="flex items-center gap-2 mb-1">
              <TrendingDown size={15} className="text-blue-600" />
              <p className="text-xs font-medium text-blue-700">Total Cost of Goods</p>
            </div>
            <p className="text-2xl font-bold text-blue-800">
              {formatKES(totals?.totalCost || 0)}
            </p>
            <p className="text-xs text-blue-600 mt-0.5">Qty sold × base cost</p>
          </div>
          ...
          <div className={`rounded-xl border p-4 ${
            ...
            <div className="flex items-center gap-2 mb-1">
              <DollarSign size={15} className={
                (totals?.netProfit || 0) >= 0 ? 'text-purple-600' : 'text-red-600'
              } />
              <p className={`text-xs font-medium ${
                (totals?.netProfit || 0) >= 0 ? 'text-purple-700' : 'text-red-700'
              }`}>Net Gross Profit</p>
            </div>
            <p className={`text-2xl font-bold ${
              (totals?.netProfit || 0) >= 0 ? 'text-purple-800' : 'text-red-800'
            }`}>
              {formatKES(totals?.netProfit || 0)}
            </p>
            <p className={`text-xs mt-0.5 ${
              (totals?.netProfit || 0) >= 0 ? 'text-purple-600' : 'text-red-600'
            }`}>
              {(totals?.marginPct || 0).toFixed(1)}% margin · actual profit made
            </p>
          </div>
        </div>

        {/* Detailed Product Profit Table */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700">Detailed Product Profit Analysis</h3>
            <p className="text-xs text-gray-400 italic">
              * Avg. selling price reflects price changes/fluctuations during period
            </p>
          </div>
          ...
          {profitData?.rows && profitData.rows.length > 0 ? (
            <div className="overflow-x-auto rounded-xl border border-gray-100">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {profitData.rows.map((row: any, i: number) => (
                    <tr key={row.productId}
                      ...
                      <td className="px-4 py-3 text-right text-gray-600">
                        {formatKES(row.currentBaseCost)}
                      </td>
                      <td className="px-4 py-3 text-right text-gray-600">
                        {formatKES(row.avgSellingPrice)}
                      </td>
                      <td className="px-4 py-3 text-right font-semibold text-gray-900">
                        {formatKES(row.totalRevenue)}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <span className={`inline-flex items-center gap-1 font-semibold text-sm ${
                          row.profitMade >= 0 ? 'text-emerald-600' : 'text-red-500'
                        }`}>
                          {row.profitMade >= 0
                            ? <TrendingUp size={13} />
                            : <TrendingDown size={13} />
                          }
                          {formatKES(row.profitMade)}
                        </span>
                ...
                <tfoot>
                  <tr className="bg-gray-50 border-t-2 border-gray-200">
                    <td className="px-4 py-3 font-bold text-gray-800" colSpan={4}>
                      TOTAL
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`font-bold text-sm ${
                        (totals?.netProfit || 0) >= 0 ? 'text-emerald-600' : 'text-red-500'
                      }`}>
                        {formatKES(totals?.netProfit || 0)}
                      </span>
                      <p className="text-xs text-gray-400 text-right mt-0.5">
                        {(totals?.marginPct || 0).toFixed(1)}% overall
                      </p>
                    </td>
                  </tr>
                </tfoot>
            ...
            <div className="flex flex-col items-center justify-center h-32 text-gray-300 gap-2">
              <TrendingUp size={28} />
              <p className="text-sm">No sales records found for this period</p>
            </div>
          )}
        </div>
      </div>
