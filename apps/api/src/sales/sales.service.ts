...
export class SalesService {
  ...
  async createSale(dto: CreateSaleDto, userId: string, pharmacyId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (change < 0) throw new BadRequestException(`Insufficient payment`);
      ...
      const sale = await tx.sale.create({
        data: {
          ...
          subtotal, vatAmount,
          discount: 0,
          totalAmount, amountPaid: dto.amountPaid, change,
          paymentMethod: dto.paymentMethod,
          notes: dto.notes,
          items: { create: saleItems },
          payments: {
            create: { method: dto.paymentMethod, amount: dto.amountPaid, status: 'SUCCESS' },
          },
        },
        include: {
  ...
  async findAll(
    branchId: string,
    query: { page?: string; limit?: string; from?: string; to?: string },
    requesterId?: string,
    ...
    if (requesterRole === 'CASHIER' || requesterRole === 'DISPENSER') {
    ...
    const [sales, total] = await Promise.all([
      this.prisma.sale.findMany({
        ...
        include: { items: true, customer: true, user: { select: { firstName: true, lastName: true } } },
  ...
  async getDailySummary(branchId: string, date?: string, userId?: string, userRole?: string) {
    ...
    if (userRole === 'CASHIER' || userRole === 'DISPENSER') {
      where.userId = userId;
    }
